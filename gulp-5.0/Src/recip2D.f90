...
!****************
  if (.not.lgrad2) then
    tsum0 = g_cpu_time()
    call sumall(erecip,sum,1_i4,"recip2D","erecip")
    esum = sum(1)
    if (lsg1) then
...
!
      tsum0 = g_cpu_time()
      call sumall(vx,sum,numat,"recip2D","vx")
      do i = 1,numat
        vx(i) = sum(i)
      enddo
      call sumall(vy,sum,numat,"recip2D","vy")
      do i = 1,numat
        vy(i) = sum(i)
      enddo
      call sumall(vz,sum,numat,"recip2D","vz")
      do i = 1,numat
        vz(i) = sum(i)
      enddo
      call sumall(vx12,sum,numat,"recip2D","vx12")
      do i = 1,numat
        vx12(i) = sum(i)
      enddo
      call sumall(vy12,sum,numat,"recip2D","vy12")
      do i = 1,numat
        vy12(i) = sum(i)
      enddo
      call sumall(vz12,sum,numat,"recip2D","vz12")
      do i = 1,numat
        vz12(i) = sum(i)
      enddo
      tsum = tsum + g_cpu_time() - tsum0
