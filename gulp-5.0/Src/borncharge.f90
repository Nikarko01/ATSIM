...
!
      ...
      do i = 1,numatr1
        if (nat(i).le.maxele) then
          n = n + 1
          bornq(1,1,n) = qf(i)*occuf(i)/dble(nprocs)
          bornq(2,1,n) = 0.0_dp
          bornq(3,1,n) = 0.0_dp
          bornq(1,2,n) = 0.0_dp
          bornq(2,2,n) = qf(i)*occuf(i)/dble(nprocs)
          bornq(3,2,n) = 0.0_dp
          bornq(1,3,n) = 0.0_dp
          bornq(2,3,n) = 0.0_dp
          bornq(3,3,n) = qf(i)*occuf(i)/dble(nprocs)
        endif
      enddo
    else
!
...
!
      ...
      do i = 1,nshellonnode
        ii = nshptr(i)
        if (nregionno(nsft+nrelat(ii)).eq.1) then
...
!**********************************
  if (nprocs.gt.1) then
    allocate(bqtmp(3,3,n),stat=status)
    if (status/=0) call outofmemory('borncharge','bqtmp')
!
    call sumall(bornq,bqtmp,9_i4*n,"borncharge","bornq")
    bornq(1:3,1:3,1:n) = bqtmp(1:3,1:3,1:n)
!
