...
!
  ...
  if (index(word,'external_p').eq.1) goto 180
...
640 if (nfloat.eq.0) then
  ...
  if (nword.ge.2) then
    if (index(words(2),'noi').eq.1) ltrannoise(ncurr) = .true.
  endif
  if (ltrannoise(ncurr)) then
    if (nfloat.ge.5) then
      xtran(ncurr) = floats(1)
      ytran(ncurr) = floats(2)
      ztran(ncurr) = floats(3)
      ntran(ncurr) = nint(floats(4))
      trannoise(ncurr) = abs(floats(5))
    else
...
!
    if (n1connect(nconnect).gt.nasym) then
      call outerror('Atom index for connect exceeds number of atoms',iline)
      call stopnow('strword')
    elseif (n2connect(nconnect).gt.nasym) then
      call outerror('Atom index for connect exceeds number of atoms',iline)
      call stopnow('strword')
    endif
