...
!
!  Outer loop replaced by i = matom
!
!  Find EAM species for i
!  
  neamspeci = neamfnspecptr(matom)
!
!  Evaluate functional derivatives
!
  if (lMEAMfn) then
    call meamfnderv(neamfn,neamspeci,scrho(1,matom),rhoi,eeam,rscrhoi,rscrhoi3,rscrhoi5,.true.,.false.,.false.)
  else
    call eamfnderv(neamfn,neamspeci,scrho(1,matom),eeam,rscrhoi,rscrhoi3,rscrhoi5,.true.,.false.,.false.)
    rhoi = scrho(1,matom)
  endif
!
  xal = xclat(matom)
...
!
    ...
    rp = sqrt(cut2)
!*************************************
!  Find valid vectors for matom - j  *
!*************************************
    call rfind(xcrd,ycrd,zcrd,cut2,0.0_dp,0.0_dp,.false.,.false.,matom,j,0_i4,0_i4,0_i4,0_i4,nmolonly,lself,0_i4,nor)
!
...
!***************************************************
      ...
      zcd = ztmp(ii)
!
!  Correct for displacements for matom 
!
      if (j.ne.matom) then
...
!
                ...
                zik0 = zclat(k) - zal
!
!  Correct for displacements unless matom = i
!
                if (k.ne.matom) then
                  xik0 = xik0 - (vmatom(1) - xclat(matom))
                  yik0 = yik0 - (vmatom(2) - yclat(matom))
                  zik0 = zik0 - (vmatom(3) - zclat(matom))
                endif
!
                xjk0 = xik0 - xcd
                yjk0 = yik0 - ycd
...
!
                call meamtotalrhoderv(neamspeci,scrho(1,matom),rhoi,drhoij,drhototij,drhoijs,drhototijs, &
