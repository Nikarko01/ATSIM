        ...
        do j = 1,numat
          if (lelementOK(nat(j)).and.nregionno(nsft+nrelat(j)).eq.1) then
            ...
            do k = 1,neemfull
              ...
              dqdxyz(indj+1,j) = dqdxyz(indj+1,j) - dchix(k)*derv2(jeem,ieem)
              dqdxyz(indj+2,j) = dqdxyz(indj+2,j) - dchiy(k)*derv2(jeem,ieem)
              dqdxyz(indj+3,j) = dqdxyz(indj+3,j) - dchiz(k)*derv2(jeem,ieem)
            enddo
          endif
        enddo
      endif
