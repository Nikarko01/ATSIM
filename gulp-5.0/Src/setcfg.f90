...
!
  ...
  use observables
  use optimisation,  only : lfix1atom, nfixatom, nfixatomtype, loldvarorder
  use radial
  use shells
...
!
  ...
  do ii = 1,ncfg
    ...
    lsuperghost = (nsuperghost(1,ii).gt.1.or.nsuperghost(2,ii).gt.1.or.nsuperghost(3,ii).gt.1)
    lsymopt = lsymset(ii)
    call setup(.false.)
...
!
      ...
      if (ndimen(ii).eq.3.and.(.not.lflags.or..not.lmc).and.(lfix1atom.and..not.nfixatomtype.eq.5)) then
!
...
!
      ...
      enddo
      ncellmaxcfg(ii) = nvarl - nvarlold
!
!  Internals
!
      ninternalmincfg(ii) = ncellmaxcfg(ii) + 1
      do j = nstrains+1,mvar
        if (ltmp(j)) then
          nvarl = nvarl + 1
          ioptcfg(nvarl) = j
        endif
...
!
      ...
      enddo
      ninternalmaxcfg(ii) = nvarl - nvarlold
    else
!########################################
...
!  Strain
!
      ncellmincfg(ii) = ninternalmaxcfg(ii) + 1
      do j = 1,nstrains
        if (ltmp(j)) then
          nvarl = nvarl + 1
          ioptcfg(nvarl) = j
