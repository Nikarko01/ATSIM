...
!
  if ((rhoi+rhoj).lt.1.0d-32) return
!
!  Common terms
!
  dtrm0dr = bpt/r0
...
!***************
    if (abs(d).gt.1.0d-12) then
      ...
      eatm = twoZ*(Ecij*strm*exptrm + (Eci*wi*xi*logtrmi + Ecj*wj*xj*logtrmj))
      if (lgrad1) then
        dstrmdr = (1.0_dp + (trm0**2)*(3.0_dp*d))*dtrm0dr
        deriv = twoZ*(Ecij*(dstrmdr - strm*dtrm0dr)*exptrm &
                + (Eci*wi*dxidr*(1.0_dp + logtrmi) + Ecj*wj*dxjdr*(1.0_dp + logtrmj)))
        if (lgrad2) then
          d2strmdr2 = (6.0_dp*trm0*d)*dtrm0dr*dtrm0dr
          deriv2 = twoZ*(Ecij*exptrm*(d2strmdr2 - 2.0_dp*dstrmdr*dtrm0dr + strm*dtrm0dr**2) &
                   + (Eci*wi*(d2xidr2*(1.0_dp + logtrmi) + dxidr*dxidr/xi + &
                      Ecj*wj*(d2xjdr2*(1.0_dp + logtrmj) + dxjdr*dxjdr/xj))))
        endif
      endif
    else
      strm = (1.0_dp + trm0)
