        ...
        if (igroup.gt.0) then
          do i = 1,igroup
            ...
            if (mcvrptr(indi+1).gt.0) then
              indj = 0
              do j = 1,ncfoc
                write(ioout,'(i6,'' x '',4x,6f10.6)') j,(vectors(indj+1,indi+k),k=1,3)
                write(ioout,'(i6,'' y '',4x,6f10.6)') j,(vectors(indj+2,indi+k),k=1,3)
                write(ioout,'(i6,'' z '',4x,6f10.6)') j,(vectors(indj+3,indi+k),k=1,3)
                indj = indj + 3
              enddo
            endif
            indi = indi + 3
