    ...
    if (lfit) then
      ...
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          ...
          elseif (nfvar(i).eq.23.or.nfvar(i).eq.24) then
            itmp(4*(nfpot(i)-1)+nfvar(i)-22) = 1
          endif
        endif
      enddo
    endif
...
!***************************
  if (nboRloc.gt.0) then
    if (lfit) then
      allocate(itmp(9*nboR),stat=status)
      if (status/=0) call outofmemory('dumpbo','itmp')
      do i = 1,9*nboR
        itmp(i) = 0
      enddo
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          if (nfvar(i).ge.5.and.nfvar(i).le.13) then
            itmp(9*(nfpot(i)-1)+nfvar(i)-4) = 1
          endif
        endif
      enddo
    endif
...
!
      if (nBOtypeR(i).eq.1) then
        ...
        if (lfit) then
          write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,g12.6,1x,i3,1x,2(g12.6,1x),3i2)')  &
            lab1,ctmp(1),lab2,ctmp(2),BOecoeffR(i),nint(BOmcoeffR(i)),BOncoeffR(i),BOlcoeffR(i),(itmp(9*(i-1)+j),j=1,3)
        else
      ...
      elseif (nBOtypeR(i).eq.2) then
        ...
        write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,g12.6,1x,i3,1x,2(g12.6,1x),''&'')')  &
          lab1,ctmp(1),lab2,ctmp(2),BOecoeffR(i),nint(BOmcoeffR(i)),BOncoeffR(i),BOlcoeffR(i)
        if (lfit) then
          write(iout,'(2x,3(g12.6,1x),6i2)') BOccoeffR(1,i),BOccoeffR(2,i),BOhcoeffR(i),(itmp(9*(i-1)+j),j=1,6)
        else
      ...
      elseif (nBOtypeR(i).eq.3) then
        ...
        if (lfit) then
          ...
          write(iout,'(2x,9i2)') (itmp(9*(i-1)+j),j=1,9)
        else
          write(iout,'(2x,6(g12.6,1x))') (BOccoeffR(j,i),j=1,5),BOhcoeffR(i)
        endif
...
!****************************
  if (nboAloc.gt.0) then
    if (lfit) then
      allocate(itmp(9*nboA),stat=status)
      if (status/=0) call outofmemory('dumpbo','itmp')
      do i = 1,9*nboA
        itmp(i) = 0
      enddo
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          if (nfvar(i).ge.14.and.nfvar(i).le.22) then
            itmp(9*(nfpot(i)-1)+nfvar(i)-13) = 1
          endif
        endif
      enddo
    endif
...
!
      if (nBOtypeA(i).eq.1) then
        ...
        if (lfit) then
          write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,g12.6,1x,i3,1x,2(g12.6,1x),3i2)')  &
            lab1,ctmp(1),lab2,ctmp(2),BOecoeffA(i),nint(BOmcoeffA(i)),BOncoeffA(i),BOlcoeffA(i),(itmp(9*(i-1)+j),j=1,3)
        else
          write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,g12.6,1x,i3,1x,2(g12.6,1x))')  &
            lab1,ctmp(1),lab2,ctmp(2),BOecoeffA(i),nint(BOmcoeffA(i)),BOncoeffA(i),BOlcoeffA(i)
        endif
      elseif (nBOtypeA(i).eq.2) then
        ...
        write(iout,'(a5,1x,a4,1x,a5,1x,a4,1x,g12.6,1x,i3,1x,2(g12.6,1x),''&'')')  &
          lab1,ctmp(1),lab2,ctmp(2),BOecoeffA(i),nint(BOmcoeffA(i)),BOncoeffA(i),BOlcoeffA(i)
        if (lfit) then
          write(iout,'(2x,3(g12.6,1x),6i2)') BOccoeffA(1,i),BOccoeffA(2,i),BOhcoeffA(i),(itmp(9*(i-1)+j),j=1,6)
        else
...
!******************************
  if (nboZloc.gt.0) then
    if (lfit) then
      ...
      do i = 1,nfit
        if (nftyp(i).eq.6) then
          if (nfvar(i).ge.25.and.nfvar(i).le.28) then
