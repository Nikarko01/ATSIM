  subroutine peigen(mcv,freq,ncore,nphonatc,ncfoc,nsfoc,iocptr,maxd2,eigr,eigi,oscstrength,ir)
!
...
!
      ...
      do i = 1,mcv
        ...
        do j = 1,ncfoc
          ...
          xir = xir + (w1(j)**2)*conjg(ceigx)*ceigx
          yir = yir + (w1(j)**2)*conjg(ceigy)*ceigy
          zir = zir + (w1(j)**2)*conjg(ceigz)*ceigz
