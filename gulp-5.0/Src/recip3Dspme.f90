...
!
          ...
          if (lsg1) then
            ...
            Vspme(1,m1+1,m2+1,m3l+1) = xrk*xrk*dkspmetrm
            Vspme(2,m1+1,m2+1,m3l+1) = yrk*yrk*dkspmetrm
            Vspme(3,m1+1,m2+1,m3l+1) = zrk*zrk*dkspmetrm
            Vspme(4,m1+1,m2+1,m3l+1) = yrk*zrk*dkspmetrm
            Vspme(5,m1+1,m2+1,m3l+1) = xrk*zrk*dkspmetrm
            Vspme(6,m1+1,m2+1,m3l+1) = xrk*yrk*dkspmetrm
          endif
        enddo
      enddo
    enddo
...
!              xdrv(i) = xdrv(i) + rqx*rmat(1,1) + rqy*rmat(2,1) + rqz*rmat(3,1)
!              ydrv(i) = ydrv(i) + rqx*rmat(1,2) + rqy*rmat(2,2) + rqz*rmat(3,2)
!              zdrv(i) = zdrv(i) + rqx*rmat(1,3) + rqy*rmat(2,3) + rqz*rmat(3,3)
            enddo
          enddo
        enddo
!
...
!****************
  if (nprocs.gt.1) then
    tsum0 = g_cpu_time()
    call sumall(erecip,sum,1_i4,"recip3Dspme","erecip")
    esum = sum(1)
    if (lsg1) then
