...
!
        write(ioout,'(/,''  OpenKIM descriptors :'',/)')
        write(ioout,'(''--------------------------------------------------------------------------------'')')
        do nkim = 1,nkimmodel
          write(ioout,'(''  Model number = '',i6)') nkim
          write(ioout,'(''--------------------------------------------------------------------------------'')')
          do icfg = 1,ncfg
            if (lkim_model_cfg_OK(nkim,icfg)) then
              write(ioout,'(a)') trim(kim_test_descriptor(nkim,icfg))
            endif
          enddo
        enddo
        write(ioout,'(''--------------------------------------------------------------------------------'')')
...
!**********************
    if (eamXcutfactor.ne.1.0_dp) then
      write(ioout,'(/,''  Manybody second derivative cross term cutoffs to be scale by = '',f10.8)') eamXcutfactor
    endif
!*******************
!  MEAM screening  *
!*******************
...
!
      ...
      if (nBOtypeR(i).eq.1) then
        ...
        if (nBOtypeR(i).eq.3) then
          write(ioout,'(a5,a1,2x,''Kuma-'',1x,3(f16.8,1x),4x,i3)') lab1,cstyp1,BOecoeffR(i),BOncoeffR(i), &
...
!
      ...
      if (nBOtypeA(i).eq.1) then
        ...
        if (nBOtypeA(i).eq.3) then
          write(ioout,'(a5,a1,2x,''Kuma-'',1x,3(f16.8,1x),4x,i3)') lab1,cstyp1,BOecoeffA(i),BOncoeffA(i), &
        ...
        elseif (nBOtypeA(i).eq.4) then
          write(ioout,'(a5,a1,2x,''MMP  '',1x,3(f16.8,1x),4x,i3)') lab1,cstyp1,BOecoeffA(i),BOncoeffA(i), &
            BOlcoeffA(i),nint(BOmcoeffA(i))
