...
!
  ...
  use g_constants,        only : angstoev, kcaltoev
  use control
  use cosmic
  use current
  use defects
...
!******************************
  if (accuracy.ne.12.0.or.nemorder.ne.4.or.nmaxcells.ne.50.or.accf1D.ne.8.0) then
  ...
  if (stepmaxrfo.ne.100.0d0) then
    write(iout,'(''stepmx rfo '',f12.6)') stepmaxrfo
  endif
...
!  Thresholds  *
!***************
  if (thresh_fc3_ind.ne.0.00001_dp.or.thresh_fc3_tot.ne.0.001_dp) then
...
!******************************
  ...
  if (lphono) then
    if (phonfile(1:1).ne.' ') then
      if (lirout) then
        write(iout,'(''output phonon ir '',a60)') phonfile(1:60)
      else
  ...
  if (lcif) then
    if (cif_dummylattice.gt.0.0_dp) then
      if (ciffile(1:1).ne.' ') then
        write(iout,'(''output cif '',a60,1x,f10.4)') ciffile(1:60),cif_dummylattice
      else
  ...
  if (llammps) then
    if (lammpsfile(1:1).ne.' ') then
      if (llammpsvar) then
        write(iout,'(''output lammps var '',a60)') lammpsfile(1:60)
      else
  ...
  if (linertia) then
    if (inertfile(1:1).ne.' ') then
      write(iout,'(''output inertia '',a60)') inertfile(1:60)
    else
  ...
  if (lkpt) then
    if (kptfile(1:1).ne.' ') then
      write(iout,'(''output kpt '',a60)') kptfile(1:60)
    else
