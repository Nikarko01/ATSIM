...
!
!  Julian Gale, Curtin University, March 2018
!
  use iochannels
  use parallel
  implicit none
...
!******************
  if (ioproc) then
#ifdef ACCELRYS
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''*                       GENERAL UTILITY LATTICE PROGRAM  v5.0                  *'')')
    ...
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''*                     BIOVIA Materials Studio 2018 Release                     *'')')
    write(ioout,'(''********************************************************************************'')')
#else
    write(ioout,'(''********************************************************************************'')')
    ...
    write(ioout,'(''*                    Curtin University, Western Australia                      *'')')
    write(ioout,'(''********************************************************************************'')')
    write(ioout,'(''* Version = 5.0.0 * Last modified =  18th April 2018                           *'')')
    write(ioout,'(''********************************************************************************'')')
#endif
  endif
!
