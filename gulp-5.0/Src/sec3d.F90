...
!
    ...
    if (ncon.eq.0) then
!
!  Reduce derv3 from nstrain to ncell
!
      do i = ncellmin,ncellmax
        ii = i - ncellmin + 1
        io = iopt(i)
        do j = ninternalmin,ninternalmax
          derv3(j,ii) = derv3(j,io)
        enddo
      enddo
    else
!
  ...
  if (lsdebug) then
    ...
    do iproc = 0,nprocs-1
      ...
      if (procid.eq.iproc) then
        write(ioout,'(2x,10(i6,3x))')(node2var(j),j=1,nvaronnode)
        do i = 1,nvar
          write(ioout,'(2x,10(f9.2))')(derv2(i,j),j=1,nvaronnode)
        enddo
        write(ioout,'(/)')
      endif
    enddo
