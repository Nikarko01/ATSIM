...
!
!  Julian Gale, CIC, Curtin University, March 2017
!
...
!  Check size of tmat
!
  if (n3a+ninternal.gt.maxn3a) then
      ...
      if (ncon.gt.0) then
        do k = 1,ncon
          ...
          if (ii.gt.nstrains) then
            io = ncfix(k) - nstrains
            if (iopt(i).eq.ii) cmat(io) = conco(k)
          endif
        enddo
      endif
!
...
!
      ...
      do i = 1,n3a
        write(ioout,'(2x,12f6.3)')(tmat(i,n3a+j),j=1,ninternal)
      enddo
      write(ioout,'(/)')
    else
!
