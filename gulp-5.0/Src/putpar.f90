...
!-----------------------
      ...
      if (nv.eq.1) then
        ...
        if (np.eq.1.or.np.eq.2.or.(np.ge.4.and.np.le.7).or.np.eq.8.or.np.eq.14.or.np.eq.17.or.np.eq.31.or.np.eq.58) then
          if (.not.lnoabs) twopot(1,nf) = abs(twopot(1,nf))
        endif
        if (lput) xf(i) = twopot(1,nf)
      elseif (nv.eq.2) then
        twopot(2,nf) = xc(i)*scale(i)
        if (np.le.2.or.np.eq.5.or.np.eq.6.or.np.eq.14.or.np.eq.16.or.np.eq.28.or.np.eq.29.or.np.eq.58) then
