  subroutine peigend(mcv,mcvloc,mcvptr,mcvrptr,freq,ncore,nphonatc,ncfoc,nsfoc, &
                     iocptr,maxd2,eigc,oscstrength,ir)
!
...
!
          call sumall(w1,w2,mcv,"peigend","w1")
          if (ioproc) then
            do j = 1,mcv
              write(59) w2(j)
            enddo
...
!
      ...
      do ii = 1,mcvloc
        ...
        ind = 0
        do j = 1,ncfoc
          xir = xir + (w1(j)**2)*conjg(eigc(ind+1,ii))*eigc(ind+1,ii)
          yir = yir + (w1(j)**2)*conjg(eigc(ind+2,ii))*eigc(ind+2,ii)
          zir = zir + (w1(j)**2)*conjg(eigc(ind+3,ii))*eigc(ind+3,ii)
...
!
    ...
    enddo
!
    call sumall(sum1,sum2,5_i4*mcv,"peigend","sum1")
!
    do i = 1,mcv
      ir(i) = sum2(i)
      irx(i) = sum2(mcv+i)
  ...
  if (lout) then
    ...
    if (igroup.gt.0) then
      do i = 1,igroup
        ...
        if (mcvrptr(indi+1).gt.0) then
          indj = 0
          do j = 1,ncfoc
            write(ioout,'(i6,'' x '',4x,6f10.6)') j,(eigc(indj+1,mcvrptr(indi+k)),k=1,3)
            write(ioout,'(i6,'' y '',4x,6f10.6)') j,(eigc(indj+2,mcvrptr(indi+k)),k=1,3)
            write(ioout,'(i6,'' z '',4x,6f10.6)') j,(eigc(indj+3,mcvrptr(indi+k)),k=1,3)
            indj = indj + 3
          enddo
        endif
        indi = indi + 3
    ...
    if (iresid.gt.0) then
      ...
      if (mcvrptr(indi+1).gt.0) then
        indj = 0
        do j = 1,ncfoc
          write(ioout,'(i6,'' x '',4x,6f10.6)') j,(eigc(indj+1,mcvrptr(indi+k)),k=1,iresid)
          write(ioout,'(i6,'' y '',4x,6f10.6)') j,(eigc(indj+2,mcvrptr(indi+k)),k=1,iresid)
          write(ioout,'(i6,'' z '',4x,6f10.6)') j,(eigc(indj+3,mcvrptr(indi+k)),k=1,iresid)
          indj = indj + 3
        enddo
      endif
      if (nprocs.gt.1) call mpbarrier
