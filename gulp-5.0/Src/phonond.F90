...
!
  ...
  if (status/=0) call outofmemory('phonond','mcvptr')
  allocate(mcvrptr(mcv),stat=status)
  if (status/=0) call outofmemory('phonond','mcvrptr')
!
  mcvrptr(1:mcv) = 0
...
!
  ...
  do i = 1,nphonatc
    ...
    mass(i) = mass(i) + rmassi
  enddo
  do i = 1,nphonatc
    if (mass(i).eq.0.0_dp) then
      call outerror('site has a mass of zero in phonond',0_i4)
      call stopnow('phonond')
    endif
  ...
  if (leigloc) then
    ...
    if (lgammaonly) then
      ...
      if (nbornstep(ncf).gt.0.and..not.lnonanal) then
        allocate(savederv2(maxeigr,mcvloc),stat=status)
      ...
      if (nbornstep(ncf).gt.0.and..not.lnonanal) then
        allocate(savederv2(maxeigr,mcvloc),stat=status)
        if (status/=0) call outofmemory('phonond','savederv2')
        allocate(savedervi(maxeigr,mcvloc),stat=status)
...
!
    if (index(keyword,'dynam').ne.0.and.index(keyword,'debu').ne.0) then
      if (ioproc) then
      ...
      do i = 1,numat
        ...
        if (iloc.gt.0) then
          ...
          iy = ix + 1
          iz = ix + 2
          write(ioout,'(12f11.6)')(dervi(j,ix),j=1,mcv+msv)
          write(ioout,'(12f11.6)')(dervi(j,iy),j=1,mcv+msv)
          write(ioout,'(12f11.6)')(dervi(j,iz),j=1,mcv+msv)
        endif
        call mpbarrier
      enddo
    endif
...
!************************
    if (leckart.and.lcluster) then
      call eckart(mcv,mcvloc,mcvptr,maxd2,derv2,maxeigr,eigr)
    endif
!*****************************
      ...
      do ii = 1,3
        ...
        do jj = 1,3
          ...
          do i = 1,nphonatonnodec
            rmassi = rmass(nphonatonnodecptr(i))
            do j = 1,nphonatc
...
!
    if (index(keyword,'dynam').ne.0) then
      if (ioproc) then
...
!
        call oscillatorstrengthg(mcv,mcvloc,mcvptr,nphonatc,nphonatptr,nphonatc,nphonatptr, &
                                 eigr,maxeigr,oscstrength)
!
!  Optionally call Raman strengths
!
        if (lraman) then
          call ramanstrength(mcv,mcvloc,mcvptr,nphonatc,nphonatptr,nphonatc,nphonatptr, &
...
!
            ...
            if (lpartofdisp) then
              call nagamma(nphonatc,nphonatc,nphonatptr,rmass,bornkloc,maxd2,derv2)
            else
              call nagamma(nphonatc,nphonatc,nphonatptr,rmass,bornk(1,ncf),maxd2,derv2)
            endif
          endif
!
...
!  Non-analytic correction for gamma point
!
              call nagamma(nphonatc,nphonatc,nphonatptr,rmass,qfrac,maxd2,derv2)
!
!  Calculate eigenvalues and eigenvectors of corrected dynamical matrix
!
              call pdiaggd(mcv,mcvloc,maxd2,derv2,maxeigr,eigr,freq(1,k-nlkpt+1),fscale,.false.,lprint,ifail)
...
!
        call peigengd(mcv,mcvloc,mcvptr,mcvrptr,freq(1,k-nlkpt+1),ncore,nphonatc,nphonatc,nphonats, &
...
!
          do nobm = 1,nobsmode
            call getvibmoded(nobsmodeptr0+nobm,maxeigr,eigr,mcvloc,mcvptr,nfitmode,overlap)
...
!
        call lower(mcv,mcvrptr,freq(1,k-nlkpt+1),nphonatc,nphonatptr,nphonatc,nphonatptr,maxeigr,eigr)
      else
!*****************************************
...
!
              ...
              if (lpartofdisp) then
                call nagammac(nphonatc,nphonatc,nphonatptr,nphonatptr,rmass,bornkloc,maxd2,derv2,dervi,xkt,ykt,zkt)
              else
                call nagammac(nphonatc,nphonatc,nphonatptr,nphonatptr,rmass,bornk(1,ncf),maxd2,derv2,dervi,xkt,ykt,zkt)
              endif
            endif
!
!  Calculate eigenvalues and eigenvectors of corrected dynamical matrix
...
!  Non-analytic correction for gamma point
!
                call nagammac(nphonatc,nphonatc,nphonatptr,nphonatptr,rmass,qfrac,maxd2,derv2,dervi,xkt,ykt,zkt)
!
!  Calculate eigenvalues and eigenvectors of corrected dynamical matrix
!
                call pdiagd(mcv,mcvloc,maxd2,derv2,dervi,maxeigc,eigc,freq(1,k-nlkpt+1),fscale,.true.,lprint,ifail)
...
!  Output frequencies / DOS / intensities
!
          call peigend(mcv,mcvloc,mcvptr,mcvrptr,freq(1,k-nlkpt+1),ncore,nphonatc,nphonatc,nphonats, &
                       nphonatptr,maxeigc,eigc,oscstrength,IRintensity(1,k-nlkpt+1)) 
!
!  Compute group velocitiies
!
          if (lgroupvelocity) then
            call groupvelocitiesd(k-nlkpt+1,mcv,mcvloc,nphonatc,eigc,maxeigc,rmass,fscale,lprint)
          endif
!
!  Lower symmetry to remove imaginary modes if selected
!
...
!
        call sumall(meanKEperatom,sum,mcv,"phonond","meanKEperatom")
        do i = 1,mcv
...
!
  if (ldendisp.and.ioproc) then
    call outphon(nlkpt,nukpt,leigloc,nphonatc)
...
!
  ...
  if (status/=0) call deallocate_error('phonond','mcvrptr')
