...
!
    if (index(keyword,'dynam').ne.0.and.index(keyword,'debu').ne.0.and.ioproc) then
...
!
            ...
            if (lpartofdisp) then
              call nagamma(ncfocg,nphonatc,iocptr,rmass,bornkloc,maxd2,derv2)
            else
              call nagamma(ncfocg,nphonatc,iocptr,rmass,bornk(1,ncf),maxd2,derv2)
            endif
          endif
!
...
!  Non-analytic correction for gamma point
!
              call nagamma(ncfocg,nphonatc,iocptr,rmass,qfrac,maxd2,derv2)
!
!  Calculate eigenvalues and eigenvectors of corrected dynamical matrix
!
...
!
        call peigeng(mcv,freq(1,k-nlkpt+1),ncore,nphonatc,ncfocg,iocptr,maxd2,eigr, &
...
!
              ...
              if (lpartofdisp) then
                call nagammac(ncfocg,nphonatc,nphonatptr,iocptr,rmass,bornkloc,maxd2,derv2,dervi,xkt,ykt,zkt)
              else
                call nagammac(ncfocg,nphonatc,nphonatptr,iocptr,rmass,bornk(1,ncf),maxd2,derv2,dervi,xkt,ykt,zkt)
              endif
            endif
!
!  Calculate eigenvalues and eigenvectors of corrected dynamical matrix
...
!  Non-analytic correction for gamma point
!
                call nagammac(ncfocg,nphonatc,nphonatptr,iocptr,rmass,qfrac,maxd2,derv2,dervi,xkt,ykt,zkt)
!
!  Calculate eigenvalues and eigenvectors of corrected dynamical matrix
!
                if (leispack_eigensolve) then
...
!
          call peigen(mcv,freq(1,k-nlkpt+1),ncore,nphonatc,ncfocg,nsfocg,iocptr,maxd2, &
                      eigr(1),eigr(maxd2*maxd2+1),oscstrength,IRintensity(1,k-nlkpt+1)) 
!
!  Compute group velocitiies
!
          if (lgroupvelocity) then
            call groupvelocities(k-nlkpt+1,mcv,ncfoc,eigr(1),eigr(maxd2*maxd2+1),maxd2,rmass,fscale,lprint)
          endif
!
!  Lower symmetry to remove imaginary modes if selected
!
