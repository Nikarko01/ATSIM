...
!
  ...
  use optimisation,   only : loptcellpar, lfix1atom, loldvarorder
  use parallel
...
!***************************
  ...
  if (index(keyword,' kjmo').ne.0.or.index(keyword,'kjmo').eq.1) then
    write(ioout,'(''*  kjmol        - print energy contributions in kJ/mol                         *'')')
  endif
  if (index(keyword,' kcal').ne.0.or.index(keyword,'kcal').eq.1) then
