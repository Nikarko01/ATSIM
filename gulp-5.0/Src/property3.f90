...
!  Matrix inversion
!
      call matrix_inversion_shells(n,1_i4,maxd2,dervi,nshell,nshellonnode,ifail)
!
      if (ifail.ne.0) then
        nwarn = nwarn + 1
...
!  Parallel
!
          allocate(tmp2(mint,6),stat=status)
          if (status/=0) call outofmemory('property3','tmp2')
          allocate(tmp2s(mint,6),stat=status)
...
!
          do i = 1,6
            do j = 1,mint
              tmp(j) = 0.0_dp
              do k = 1,mint
                tmp(j) = tmp(j) + dervi(k,j)*derv3(k,i)
              enddo
            enddo
            do j = 1,6
              do k = 1,mint
                elcon(j,i) = elcon(j,i) - derv3(k,j)*tmp(k)
              enddo
            enddo
          enddo
        endif
...
!****************************
  if (lpiezo) then
    if (linternal) then
      allocate(tmp2(max(mint+nbs,n),6),stat=status)
      if (status/=0) call outofmemory('property3','tmp2')
!
