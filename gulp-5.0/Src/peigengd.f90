  subroutine peigengd(mcv,mcvloc,mcvptr,mcvrptr,freq,ncore,nphonatc,ncfoc,nsfoc, &
  ...
  if (lout) then
    ...
    if (igroup.gt.0) then
      do i = 1,igroup
        ...
        if (mcvrptr(indi+1).gt.0) then
          indj = 0
          do j = 1,ncfoc
            write(ioout,'(i6,'' x '',4x,3f10.6)') j,(eigr(indj+1,mcvrptr(indi+k)),k=1,3)
            write(ioout,'(i6,'' y '',4x,3f10.6)') j,(eigr(indj+2,mcvrptr(indi+k)),k=1,3)
            write(ioout,'(i6,'' z '',4x,3f10.6)') j,(eigr(indj+3,mcvrptr(indi+k)),k=1,3)
            indj = indj + 3
          enddo
        endif
        indi = indi + 3
    ...
    if (iresid.gt.0) then
      ...
      if (mcvrptr(indi+1).gt.0) then
        indj = 0
        do j = 1,ncfoc
          write(ioout,'(i6,'' x '',4x,3f10.6)') j,(eigr(indj+1,mcvrptr(indi+k)),k=1,iresid)
          write(ioout,'(i6,'' y '',4x,3f10.6)') j,(eigr(indj+2,mcvrptr(indi+k)),k=1,iresid)
          write(ioout,'(i6,'' z '',4x,3f10.6)') j,(eigr(indj+3,mcvrptr(indi+k)),k=1,iresid)
          indj = indj + 3
        enddo
      endif
      if (nprocs.gt.1) call mpbarrier
