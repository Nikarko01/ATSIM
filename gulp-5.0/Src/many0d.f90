...
!
  ...
  if (lgrad2) then
    ...
    if (status/=0) call outofmemory('many0d','npotikptr')
    allocate(npotjkptr(npote),stat=status)
    if (status/=0) call outofmemory('many0d','npotjkptr')
  endif
!
!  Set up cutoffs
!
...
!
  if (lgrad2) then
    ...
    if (status/=0) call deallocate_error('many0d','npotjkptr')
    deallocate(npotikptr,stat=status)
    if (status/=0) call deallocate_error('many0d','npotikptr')
  endif
