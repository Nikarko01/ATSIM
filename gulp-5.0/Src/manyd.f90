...
!
                if (nptype(npot).eq.45.or.nptype(npot).eq.55) then
                  ...
                  if (lstr) then
                    derivs(1) = derivs(1) + d1bas*xcd*xcd*Sij
                    derivs(2) = derivs(2) + d1bas*ycd*ycd*Sij
                    derivs(3) = derivs(3) + d1bas*zcd*zcd*Sij
                    derivs(4) = derivs(4) + d1bas*ycd*zcd*Sij
                    derivs(5) = derivs(5) + d1bas*xcd*zcd*Sij
                    derivs(6) = derivs(6) + d1bas*xcd*ycd*Sij
                  endif
!
...
!
                        ...
                        if (lstr) then
                          do kl = 1,nstrains
                            ks = nstrptr(kl)
                            rstrl(kl) = rstrl(kl) + partial%sa_drhotots(ks,np1)*ofct
                          enddo
...
!*****************************
          if (lsg1.or.lgrad2) then
            ...
            do kl = 1,nstrains
              ks = nstrptr(kl)
              rstrl(kl) = rstrl(kl) + derivs(ks)*oij
            enddo
...
!
            ...
            if (lstr) then
              drhototikssum(1:6) = 0.0_dp
              drhototjkssum(1:6) = 0.0_dp
              drhototijk2ssum(1:6,1:6) = 0.0_dp
              drhototjik2ssum(1:6,1:6) = 0.0_dp
              drhototijk2msum(1:6,1:3) = 0.0_dp
              drhototjik2msum(1:6,1:3) = 0.0_dp
            endif
!
            do jj = 1,nor2
              rik2 = dist(nor+jj)
...
!
              ...
              if (lMEAM) then
                ...
                do kl = 1,nstrains
                  ...
                  do km = 1,nstrains
                    kt = nstrptr(km)
                    elcom(km,kl) = elcom(km,kl) + dt1l*drhototikssum(ks)*drhototijs(kt)
                    elcom(km,kl) = elcom(km,kl) + dt3l*drhototijk2ssum(kt,ks)
                  enddo
                enddo
              else
                do kl = 1,nstrains
              ...
              if (lMEAM) then
                ...
                do kl = 1,nstrains
                  ...
                  do km = 1,nstrains
                    kt = nstrptr(km)
                    elcom(km,kl) = elcom(km,kl) + dt2l*drhototjkssum(ks)*drhototjis(kt)
                    elcom(km,kl) = elcom(km,kl) + dt4l*drhototjik2ssum(kt,ks)
                  enddo
                enddo
              else
                do kl = 1,nstrains
...
!  
  if (.not.lfreeze) then
    if (lMEAM) then
      if (lsg1) then
        do i = 1,6
          rstrdnr(i) = rstrdnr(i) + 0.25_dp*rstrl(i)
        enddo
      endif
    else
      if (lsg1) then
    ...
    if (lMEAM) then
      if (lsg1) then
        do i = 1,6
          rstrdnr(i) = rstrdnr(i) + 0.5_dp*rstrl(i)
        enddo
      endif
    else
      if (lsg1) then
