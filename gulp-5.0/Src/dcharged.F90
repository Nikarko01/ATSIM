  subroutine dcharged(lprint,lrecalcA,lstrain)
!
...
!
    ...
    if (lnoqeem) then
      ...
      if (lqeq.and.lhpresent) then
        call genpotdm(derv2,maxd2,chitmp,2_i4)
      else
        call genpotdm(derv2,maxd2,chitmp,1_i4)
      endif
    endif
!
    ...
    if (lqeq) then
      ...
      do iloc = 1,natomsonnode
        ...
        if (lelementOK(nat(i)).and.nregionno(nsft+nrelat(i)).eq.1) then
          ...
          if (nat(i).ne.1) then
            derv2(ieemf,ieem) = derv2(ieemf,ieem) + 2.0_dp*qeqmu(nat(i))*occuf(i)
          else
!
...
!
            ...
            derv2(ieemf,ieem) = derv2(ieemf,ieem) + 2.0_dp*qeqmu(1)*occuf(i)*rjfac
          endif 
        endif 
      enddo
    elseif (lSandM) then
      ...
      do iloc = 1,natomsonnode
        ...
        if (lelementOK(nat(i)).and.nregionno(nsft+nrelat(i)).eq.1) then
          ieem = ieem + 1
          ieemf = neemfullrptr(i)
          derv2(ieemf,ieem) = derv2(ieemf,ieem) + 2.0_dp*smmu(nat(i))*occuf(i)
        endif
      enddo
    elseif (lpacha) then
      ieem = 0
      do iloc = 1,natomsonnode
        ...
        if (lelementOK(nat(i)).and.nregionno(nsft+nrelat(i)).eq.1) then
          ieem = ieem + 1
          ieemf = neemfullrptr(i)
          derv2(ieemf,ieem) = derv2(ieemf,ieem) + 2.0_dp*mu_pacha(nat(i))*occuf(i)
        endif
      enddo
    else
      ieem = 0
      do iloc = 1,natomsonnode
        ...
        if (lelementOK(nat(i)).and.nregionno(nsft+nrelat(i)).eq.1) then
          ieem = ieem + 1
          ieemf = neemfullrptr(i)
          derv2(ieemf,ieem) = derv2(ieemf,ieem) + 2.0_dp*rmu(nat(i))*occuf(i)
        endif
      enddo
    endif
!
...
!
    ...
    do iloc = 1,natomsonnode
      ...
      if (lelementOK(nat(i)).and.nregionno(nsft+nrelat(i)).eq.1) then
        ieem = ieem + 1
        ieemf = neemfullrptr(i)
        derv2(ieemf,neemfullloc+1) = occuf(i)
        derv2(neemfull+1,ieem) = 1.0_dp
      endif
    enddo
    derv2(neemfull+1,neemfullloc+1) = 0.0_dp
!     
!  Matrix inversion 
!     
    n = neemfull + 1
          ...
          do iv = 1,nkvec
            ...
            if (lstrain) then
              if (lelementOK(nat(i)).and.nregionno(nsft+nrelat(i)).eq.1) then
                ...
                dqs(ieemf,1) = dqs(ieemf,1) - strm1*xrk(iv)*xrk(iv) - strm2
                dqs(ieemf,2) = dqs(ieemf,2) - strm1*yrk(iv)*yrk(iv) - strm2
                dqs(ieemf,3) = dqs(ieemf,3) - strm1*zrk(iv)*zrk(iv) - strm2
                dqs(ieemf,4) = dqs(ieemf,4) - strm1*yrk(iv)*zrk(iv)
                dqs(ieemf,5) = dqs(ieemf,5) - strm1*xrk(iv)*zrk(iv)
                dqs(ieemf,6) = dqs(ieemf,6) - strm1*xrk(iv)*yrk(iv)
              endif
            endif
          enddo
        elseif (ndim.eq.2) then
              ...
              if (kvec.le.Gmax) then
                ...
                if (lstrain) then
                  if (lelementOK(nat(i)).and.nregionno(nsft+nrelat(i)).eq.1) then
                    ...
                    dqs(ieemf,1) = dqs(ieemf,1) - strm1*xrk(iv)*xrk(iv) - strm2
                    dqs(ieemf,2) = dqs(ieemf,2) - strm1*yrk(iv)*yrk(iv) - strm2
                    dqs(ieemf,3) = dqs(ieemf,3) - strm1*xrk(iv)*yrk(iv)                                                    
                  endif
                endif
              endif
            enddo
...
!
    call pdgemm('t','n',numat,numat,numat,1.0d0,dqx,1,1,idesq,derv2,1,1,idesd,0.0d0,dqtmp,1,1,idest)
    dqx(1:numat,1:natomsonnode) = dqtmp(1:numat,1:natomsonnode)
    call pdgemm('t','n',numat,numat,numat,1.0d0,dqy,1,1,idesq,derv2,1,1,idesd,0.0d0,dqtmp,1,1,idest)
    dqy(1:numat,1:natomsonnode) = dqtmp(1:numat,1:natomsonnode)
    call pdgemm('t','n',numat,numat,numat,1.0d0,dqz,1,1,idesq,derv2,1,1,idesd,0.0d0,dqtmp,1,1,idest)
...
!
                  if (ndim.eq.3) then
                    dqs(ieemf,1) = dqs(ieemf,1) + qljj*dtrm1i*rxk*rxk
                    dqs(ieemf,2) = dqs(ieemf,2) + qljj*dtrm1i*ryk*ryk
                    dqs(ieemf,3) = dqs(ieemf,3) + qljj*dtrm1i*rzk*rzk
                    dqs(ieemf,4) = dqs(ieemf,4) + qljj*dtrm1i*ryk*rzk
                    dqs(ieemf,5) = dqs(ieemf,5) + qljj*dtrm1i*rxk*rzk
                    dqs(ieemf,6) = dqs(ieemf,6) + qljj*dtrm1i*rxk*ryk
                  elseif (ndim.eq.2) then
                    dqs(ieemf,1) = dqs(ieemf,1) + qljj*dtrm1i*rxk*rxk
                    dqs(ieemf,2) = dqs(ieemf,2) + qljj*dtrm1i*ryk*ryk
                    dqs(ieemf,3) = dqs(ieemf,3) + qljj*dtrm1i*rxk*ryk
                  endif
                  if (lSandM) then
                    if (ndim.eq.3) then
                      dchis(1,iloc) = dchis(1,iloc) + dtrm1zn*rxk*rxk
                      dchis(2,iloc) = dchis(2,iloc) + dtrm1zn*ryk*ryk
                      dchis(3,iloc) = dchis(3,iloc) + dtrm1zn*rzk*rzk
                      dchis(4,iloc) = dchis(4,iloc) + dtrm1zn*ryk*rzk
                      dchis(5,iloc) = dchis(5,iloc) + dtrm1zn*rxk*rzk
                      dchis(6,iloc) = dchis(6,iloc) + dtrm1zn*rxk*ryk
                    elseif (ndim.eq.2) then
                      dchis(1,iloc) = dchis(1,iloc) + dtrm1zn*rxk*rxk
                      dchis(2,iloc) = dchis(2,iloc) + dtrm1zn*ryk*ryk
                      dchis(3,iloc) = dchis(3,iloc) + dtrm1zn*rxk*ryk
                    endif
                  endif
                endif
              endif
        ...
        do j = 1,numat
          if (lelementOK(nat(j)).and.nregionno(nsft+nrelat(j)).eq.1) then
            ...
            do k = 1,neemfull
              ...
              dsmtrm(indj+1,j) = dsmtrm(indj+1,j) - dchix(k)*derv2(jeem,ieem)
              dsmtrm(indj+2,j) = dsmtrm(indj+2,j) - dchiy(k)*derv2(jeem,ieem)
              dsmtrm(indj+3,j) = dsmtrm(indj+3,j) - dchiz(k)*derv2(jeem,ieem)
            enddo
          endif
        enddo
      endif
...
!
        ...
        do j = 1,numat
          if (lelementOK(nat(j)).and.nregionno(nsft+nrelat(j)).eq.1) then
            jeem = jeem + 1
            indj = 3*(j - 1)
            dqdxyz(indj+1,iloc) = dqdxyz(indj+1,iloc) - fieldx*derv2(jeem,ieem)
            dqdxyz(indj+2,iloc) = dqdxyz(indj+2,iloc) - fieldy*derv2(jeem,ieem)
            dqdxyz(indj+3,iloc) = dqdxyz(indj+3,iloc) - fieldz*derv2(jeem,ieem)
          endif
        enddo
      endif
!
...
!
    if (lstrain) then
      if (lSandM) then
        call pdgemm('n','n',6,numat,numat,1.0d0,dchis,1,1,idess,derv2,1,1,idesd,0.0d0,dstmp,1,1,idess) 
...
!             
              if (lelementOK(nat(i)).and.nregionno(nsft+nrelat(i)).eq.1) then
                dqs(ieemf,1) = dqs(ieemf,1) + qljj*dtrm1i*rx*rx
                if (lSandM) then
                  dchis(1,iloc) = dchis(1,iloc) + dtrm1zn*rx*rx
                endif
              endif
            endif
          enddo
...
!
    if (lstrain) then
      if (lSandM) then
        call pdgemm('n','n',1,numat,numat,1.0d0,dchis,1,1,idess,derv2,1,1,idesd,0.0d0,dstmp,1,1,idess)
