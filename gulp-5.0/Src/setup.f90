...
!   7/17 Call to setoptptr moved to optim since it needs lopf
!        to be set first
!   8/17 Parallel decomposition now only computed for call where lall is true
!        otherwise blocksize trap can be applied to cell read in instead of
!        any supercell
!   8/17 Order of calling setmol(s) and parallel initialisation switched to 
...
!
  ...
  use molecule
  use optimisation,  only : nfixatom, nfixatomtype
  use parallel
  use partial
  use polarise
  use reallocate
...
!
  ...
  if (((leem.or.lqeq.or.lSandM.or.lpacha).and.(.not.lnoqeem)).or.(nboQ.gt.0)) lewald = .true.
...
!
  do i = 1,nvar
    ...
    if (iopt(i).gt.3*nasym+nstrains) then
      nbsm = nbsm + 1
    endif
  enddo
  if (lall) then
