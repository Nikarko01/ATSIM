...
\item thermal conductivity (output of files for ShengBTE to use Boltzmann Transport Equations)
...
\item DLV (.str)
\item ShengBTE files (CONTROL, FORCE\_CONSTANTS\_2ND, FORCE\_CONSTANTS\_3RD)
...
\item Externally defined potentials via OpenKIM
\end{itemize}
...
an Eckart transformation \cite{Eckart}. 
...
k vector at a given point in reciprocal space. They are useful in describing
phonon propagation through a solid and can now be computed by GULP. 
...
GULP. However, GULP is able to output the three files needed to interface to a separate standalone code,
ShengBTE \cite{shengbte}, that is freely available and able to compute the thermal conductivity, as well as other properties.
The files that are output are CONTROL, FORCE\_CONSTANTS\_2ND, and FORCE\_CONSTANTS\_3RD. The latter
two of these files are the second and third derivative matrices computed by GULP, while CONTROL is the file that provides
the system information and other parameters needed to solve the BTE by ShengBTE. It is important to note here that the 
force constant matrices need to be output for a supercell (if the unit cell is small) to ensure that the real space force constants
are captured out to a given range. If the supercell size is too small then force constants between translational images will 
be summed together and therefore incorrectly phased away from the gamma point. The size of the supercell to be used
for the ShengBTE calculation is set using the "supercell" option. In a restart file then the previous cell size is captured using
the "ghost\_supercell" option for the benefit of ShengBTE. For more details regarding ShengBTE see the web site for this software (http://www.shengbte.org).
...
\texttt{setenv GULP\_DOC /Users/myname/gulp5.0/Docs}

\texttt{setenv GULP\_LIB /Users/myname/gulp5.0/Libraries}~\\
\texttt{}~\\
...
{\texttt{groupvelocity}}&calculation the group velocities at each k point\\
...
{\texttt{threshold       }}&specifies a drop tolerance for third derivative matrix elements in the output to ShengBTE\\
...
\noindent \textbf{(a) Control of calculation type}\\
 \texttt{angle, bond, cosmo, cosmic, cost, defect, distance, eem, efg, eharmonic, fit, free\_energy,
gasteiger, genetic, gradients, groupvelocity, md, montecarlo, noautobond, noenergy,
optimise, pacha, pot, predict, preserve\_Q, property, phonon, qeq, qbond,
raman, single, sm, static\_first, thermalconductivity, torsion, transition\_state}~\\
...
\noindent \textbf{(c) Algorithm} \\
 \texttt{c6, dipole, fbfgs, fix\_molecule, full, ghostcell, hill, kfull, marvinSE, madelung,
...
nokpoints, operators, outcon, prt\_eam, prt\_two, prt\_three, prt\_four, prt\_six, prt\_regi\_before, qsas,
restore, save, site\_energy, stress\_out, terse}~\\
...
{\texttt{ghost\_supercell}}&specifies the size of the underlying supercell for the benefit of output to ShengBTE\\
...
shengBTE &  & writes out 3 files needed for input to ShengBTE \tabularnewline
\hline
\end{tabular}
...
\\
\texttt{setenv GULP\_LIB /Users/julian/progs/gulp5.0/Libraries }~\\
\texttt{setenv GULP\_DOC /Users/julian/progs/gulp5.0/Docs}\\
...
-k => needs OpenKIM (http://openkim.org)\\
